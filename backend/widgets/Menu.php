...
class Menu extends \yii\widgets\Menu
{
     ...
     */
    public $labelTemplate = '<cite>{label}</cite>';
    public $submenuTemplate = "\n<dl class='layui-nav-child' {show}>\n{items}\n</dl>\n";
    public $activateParents = true;
    public $defaultIconHtml = '<i class="icon iconfont">&#xe658;</i> ';
	
	public $defaultIconHtmlIcon = '<i class="icon fa fa-heart-o"></i> ';
    ...
    protected function renderItem($item)
    {
        if (isset($item['items'])) {
            ...
            $linkTemplate = '<a href="{url}" lay-tips="'.$item['label'].'" lay-direction="2">{icon} {label}</a>';
        } else {
    ...
    protected function renderItems($items)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            $tag = ArrayHelper::remove($options, 'tag', 'dd');
            $class = [];
            if ($item['active']) {
            ...
            $menu = mb_ereg_replace('href=""','',$menu);
            if (!empty($item['items'])) {
				...
				if (isset($options['class'])) {
					$options['class'] .= 'layui-nav-item layui-nav-itemed';
				} else {
				    //默认系统管理栏目展开
                    if(isset($item['label']) && $item['label'] == '系统管理'){
                        $options['class'] = 'layui-nav-item layui-nav-itemed';
                    }else{
