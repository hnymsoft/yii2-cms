...
class MenuHelper
{
    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false)
    {
        ...
        $menus = Menu::find()->where(['is_show'=>1])->asArray()->indexBy('id')->all();
        ...
        if ($refresh || $cache === null || ($assigned = $cache->get($key)) === false) {
            ...
            $assigned = [];
            $query = Menu::find()->select(['id'])->where(['is_show'=>1])->asArray();
            if (count($filter2)) {
                $assigned = $query->where(['route' => $filter2,'is_show'=>1])->column();
            }
            if (count($filter1)) {
                $query->where('route like :filter')->andWhere(['is_show'=>1]);
